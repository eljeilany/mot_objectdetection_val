...
from mot.object_detection.dataset import DatasetRegistry, register_mot, get_class_names, MotDataset
...
def register_mot_loc(
    ...
    def dataset(x):
        mot = MotDataset(base_dir, split=x)
        mot.ratio = .86
        return mot
    for split in ["train", "val"]:
        name = "mot_" + split
        DatasetRegistry.register(name, lambda x=split: dataset(x))
...
if __name__ == '__main__':
    ...
    print(cfg)
    cfg.freeze(False)
#     cfg.update_args = update_args
    if args.config:
        update_args(cfg, args.config)
    register_mot_loc(cfg.DATA.BASEDIR) # add the mot datasets to the registry
